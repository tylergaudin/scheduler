 ...
 */
public class SaveFormUpdateController implements Initializable {
    ...
    {
        SaveFormUpdateController.userAppointment = userAppointment;
    }

    public static User mainUser = null;

    public static void setMainUser(User mainUser) {
        SaveFormUpdateController.mainUser = mainUser;
    }

    private int appointmentId = userAppointment.getAppointmentId();
        ...
        for (User user: users)
        {
            if(user.getUserId() == mainUser.getUserId())
            {
                if (userNameTxt.getText().contains(user.getUserName())
                    ...
                    AppointmentsQuery.update(appointmentId, title, description, location, type, start,
